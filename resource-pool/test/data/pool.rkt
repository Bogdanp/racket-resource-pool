    ...
    (run-tests
     (test-suite
      ...
      (test-suite
       ...
       (test-case "initial stats"
         (define p (make-pool #:max-size 4 gensym))
         (define s (pool-stats p))
         (check-equal? (pool-stats-open s) 0)
         (check-equal? (pool-stats-busy s) 0)
         (check-equal? (pool-stats-idle s) 0)
         (pool-close! p))

       (test-case "stats after taking resources"
         (define p (make-pool #:max-size 4 gensym))
         (define v1 (pool-take! p))
         (define s1 (pool-stats p))
         (check-equal? (pool-stats-open s1) 1)
         (check-equal? (pool-stats-busy s1) 1)
         (check-equal? (pool-stats-idle s1) 0)
         (define v2 (pool-take! p))
         (define s2 (pool-stats p))
         (check-equal? (pool-stats-open s2) 2)
         (check-equal? (pool-stats-busy s2) 2)
         (check-equal? (pool-stats-idle s2) 0)
         (pool-release! p v1)
         (pool-release! p v2)
         (pool-close! p))

       (test-case "stats after releasing resources"
         (define p (make-pool #:max-size 4 gensym))
         (define v1 (pool-take! p))
         (define v2 (pool-take! p))
         (pool-release! p v1)
         (define s (pool-stats p))
         (check-equal? (pool-stats-open s) 2)
         (check-equal? (pool-stats-busy s) 1)
         (check-equal? (pool-stats-idle s) 1)
         (pool-release! p v2)
         (define s2 (pool-stats p))
         (check-equal? (pool-stats-open s2) 2)
         (check-equal? (pool-stats-busy s2) 0)
         (check-equal? (pool-stats-idle s2) 2)
         (pool-close! p))

       (test-case "stats after abandoning resources"
         (define p (make-pool #:max-size 4 gensym))
         (define v1 (pool-take! p))
         (define v2 (pool-take! p))
         (pool-abandon! p v1)
         (define s (pool-stats p))
         (check-equal? (pool-stats-open s) 1)
         (check-equal? (pool-stats-busy s) 1)
         (check-equal? (pool-stats-idle s) 0)
         (pool-release! p v2)
         (pool-close! p))

       (test-case "stats at pool capacity"
         (define p (make-pool #:max-size 2 gensym))
         (define v1 (pool-take! p))
         (define v2 (pool-take! p))
         (define s (pool-stats p))
         (check-equal? (pool-stats-open s) 2)
         (check-equal? (pool-stats-busy s) 2)
         (check-equal? (pool-stats-idle s) 0)
         (pool-release! p v1)
         (pool-release! p v2)
         (pool-close! p))

       (test-case "stats with reused resources"
         (define p (make-pool #:max-size 2 gensym))
         (define v1 (pool-take! p))
         (pool-release! p v1)
         (define v2 (pool-take! p))
         (define s (pool-stats p))
         (check-equal? (pool-stats-open s) 1 "should reuse released resource")
         (check-equal? (pool-stats-busy s) 1)
         (check-equal? (pool-stats-idle s) 0)
         (pool-release! p v2)
         (pool-close! p))

       (test-case "stats invariant: open = busy + idle"
         (define p (make-pool #:max-size 5 gensym))
         (define-check (check-invariant)
           (define s (pool-stats p))
           (check-equal?
            (pool-stats-open s)
            (+ (pool-stats-busy s)
               (pool-stats-idle s))
            "open should equal busy + idle"))
         (check-invariant)
         (define v1 (pool-take! p))
         (check-invariant)
         (define v2 (pool-take! p))
       ...
       (test-case "stats after make-resource failure"
         (define fail? #t)
         (define p
           (make-pool
            #:max-size 2
            (lambda ()
              (if fail?
                  (error 'fail)
                  (gensym)))))
         (check-exn #rx"fail" (lambda () (pool-take! p)))
         (define s1 (pool-stats p))
         (check-equal? (pool-stats-open s1) 0 "failed resource should not count as open")
         (check-equal? (pool-stats-busy s1) 0)
         (check-equal? (pool-stats-idle s1) 0)
         (set! fail? #f)
         (define v (pool-take! p))
         (define s2 (pool-stats p))
         (check-equal? (pool-stats-open s2) 1)
         (check-equal? (pool-stats-busy s2) 1)
         (check-equal? (pool-stats-idle s2) 0)
         (pool-release! p v)
         (pool-close! p)))))))
